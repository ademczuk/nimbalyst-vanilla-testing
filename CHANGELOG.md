...
### Added
- LaTeX math editing and rendering inside the Lexical document editor. The new built-in `math` extension renders inline (`$...$`) and block (`$$...$$`) math nodes through KaTeX and supports double-click editing with live preview for both inline and block formulas. The Electron extension loader now exposes extension Lexical contributions so extensions can register their own custom node types, decorators, and transformers; the math rendering that previously lived directly inside the agent transcript markdown pipeline moves into the same extension contribution so transcript math and document math share one code path. Includes E2E coverage and Electron test support for the math extension.
- Programmable action prompts in the AI composer. An Actions dropdown on the composer's controls row lists reusable prompt presets defined per workspace in `nimbalyst-local/ai-actions.md`. Selecting an action replaces the draft with its body verbatim, places the cursor at the end, and pushes a boundary undo snapshot so `Cmd+Z` restores the prior draft. Parser splits on level-2 headings and preserves bodies verbatim (including slash commands and code fences) with diagnostics for duplicate or empty headings. A per-workspace service caches the parsed list and invalidates on file changes via the workspace event bus; `ensureFileExists` seeds a default template with four example actions. The dropdown stays live through an atom family fed by a central IPC listener (per the IPC listeners pattern); an empty-state CTA lets the user create and open `ai-actions.md` from inside the dropdown. PostHog `action_prompt_inserted` event tracks usage. Parser / service unit tests + an E2E spec ship with the feature.
- Collaborative editing for extension custom editors. New `useCollaborativeEditor` hook and `CollaborationContext` in `@nimbalyst/extension-sdk` (0.2.0) plus a `customEditors[].collaboration` manifest field let any custom editor opt into real-time collaboration. Behaviour-preserving extraction of provider lifecycle, awareness, and key rotation into `CollaborativeTabEditor` with an extension dispatch branch (Lexical path unchanged). Share-to-Team generalizes the collab index entry with `documentType` so non-markdown files route to the right editor on the recipient side. `DocumentSync` adds client-side yJS snapshot/compaction with single-elector coordination to keep initial sync fast as histories grow. Built-in editors that opt in: Excalidraw (elements as `Y.Map` keyed by element id, fractional-indexing order, awareness -> collaborators) and CSV spreadsheet (single `Y.Text` binding with awareness for `selectedCell` / `editingCell`). New `COLLABORATION_GUIDE.md` plus `EXTENSION_ARCHITECTURE.md` and `FEATURE_INVENTORY.md` updates document the contract.
- MockupLM Share-to-Team collaborative editing for `.mockup.html` and `.mockupproject` files. A single `Y.Text` binding syncs `.mockup.html` across teammates so remote edits update the iframe and awareness carries element selection; a keyed-entity `Y.Map` binding syncs `.mockupproject` canvas so mockups, connection edges, and meta (name/viewport) all converge per-field instead of clobbering each other on concurrent edits. `Y.UndoManager` hijacks `Cmd/Ctrl+Z` so undo only touches local ops. Deterministic seeds use the on-disk mockup/connection ids verbatim so two clients racing the initial seed don't produce duplicates; `applyContent` short-circuits in collab mode so a late `loadContent()` resolution can't overwrite the live `Y.Doc`; `seedMockupProjectYDoc` bails on empty content and `isEmpty` includes meta so a recipient with no `initialContent` can't clobber the shared project name/viewport with defaults. Selection awareness snapshot updates after the publish, not before, so `selectedMockupId` actually propagates past the first tick.
- Tracker sync rewritten as a metadata-layer `TrackerSyncEngine`. Server-assigned monotonic `syncId` per room with clients reconnecting from their last-seen value to receive a delta stream. Hybrid conflict resolution: server-LWW for scalars, add-wins set for labels, append-only log for comments, and `Y.Doc` CRDT for bodies. Bootstrap with partial-by-type pagination via `hasMore` + `cursorSyncId`; bodies lazy-loaded from `DocumentRoom` and cached locally in `tracker_body_cache`. Four-state transaction queue persisted to PGLite (`pendingApply -> created -> queued -> executing -> persistedEnqueue`) with crash-safe replay on bootstrap. E2E encryption envelope: `itemId` / `syncId` / `deletedAt` / `updatedAt` plaintext, everything else inside AES-256-GCM ciphertext; tombstones are `encryptedPayload: null` + `deletedAt` set. Stable routing identity: `teamProjectId` is a server-minted UUID in `TeamRoom`; rooms are named `org:{orgId}:tracker:{uuid}`. Adds the body `Y.Doc` cache pipeline (`BodyDocCache`, `MainBodyDocService`, `HeadlessLexicalYDoc`, `useTrackerBodyPrewarm`) so MCP body writes and detail-panel reads project through one `Y.Doc` per item. Two collaborative E2E specs updated to drive the new envelope.
- Install-from-GitHub now prefers Release assets over cloning source. The marketplace install flow tries the repo's latest Release first and downloads a `.nimext` or `.zip` asset before falling back to cloning HEAD and looking for committed `dist/`. Authors who publish a Release no longer need to commit `dist/` to source. Subdir URLs match release assets by name prefix; falls through to clone-source on no match. Present-but-broken release assets hard-fail rather than silently installing the source-based version. The install flow streams stage updates ("Checking for release artifact...", "Downloading X (tag)...", "No release artifact, cloning source...") to the renderer over a new `install-progress` IPC event, surfaced via a central listener and Jotai atom per the existing IPC pattern. Shared download/extract/validate logic extracted into `installFromPackageUrl`; staging-then-rename avoids wiping an existing install if the new package is broken. `selectReleaseAsset` lives in its own module with 13 unit tests covering the selection rules.
- `nimbalyst-settings` MCP server for in-app configuration. Single-writer `SettingsControlService` enforces an allowlist of app/workspace keys, an asserted-in-test denylist for secret-bearing keys, per-session rate limiting, and audit logging. The new MCP server exposes tools for theme, sync, feature flags, extensions, tracker policies, and workspace lifecycle. Wired into all AI provider MCP configs so agents can apply settings consistently across Claude Code, Codex, OpenCode, and Copilot CLI. `sync_set_for_project` recomputes `sessionSync.enabled` from the final project set so disabling the last project turns global sync off. `appearance_set_theme` broadcasts on the `theme-change` channel and runs the full theme side effects (native theme, title bars, pending fallback) so the live UI reflects the change. `ai_set_preferred_language` pushes the new value into the `SessionNamingService` singleton so providers pick it up immediately.
- Transcript markdown and tool widgets are now extensible by runtime contributions; the previously hardcoded transcript math rendering moves into the new built-in `math` extension via this contribution surface.

### Changed
- `@nimbalyst/collab-protocol` extracted as the single source of truth for the client<->server WebSocket wire protocol, replacing the duplicated unions that previously lived in both `packages/collabv3/src/types.ts` and `packages/runtime/src/sync/*Types.ts`. `runtime/src/sync` now imports wire types from `collab-protocol`; `teamSyncTypes` / `documentSyncTypes` / `ProjectSyncProvider` keep only their client-side config and decrypted projections. Deleted `packages/collabv3` and `packages/collabv3-metrics`; both moved to a separate `nimbalyst-collab` project. `packages/electron/e2e/utils/wranglerHelpers.ts` resolves the collab server path via `COLLAB_SERVER_PATH` (default `../nimbalyst-collab/packages/collabv3`) so the gated `RUN_COLLAB_TESTS=1` E2E specs still work against a sibling checkout.
- `extension-sdk` released as 0.2.0 with the new collab editor API (`useCollaborativeEditor`, `CollaborationContext`, `customEditors[].collaboration` manifest field). Backwards-compatible: the hook no-ops on hosts without `host.collaboration`, so 0.2.0 extensions still load and edit locally on older Nimbalyst hosts (`minAppVersion` stays 0.58.5).
- Agent transcript returned to virtua with safer selection handling. The main transcript path uses virtua again instead of the v0.60.3/v0.60.4 flat-list path; the desktop render buffer widens to reduce row churn near selection; the `keepMounted` selection-pinning experiment is dropped after it destabilized browser selection.
- Wakeup-triggered queued prompts no longer render in the user lane with a "You" header. ScheduleWakeup-originated prompts are tagged with `promptOrigin='wakeup_resume'` and threaded through `documentContext` into raw-message metadata; `ClaudeCodeRawParser` emits a `system_message` with `reminderKind='wakeup_resume'` instead of `user_message`, and `RichTranscriptView` labels the card "Resumed from scheduled wakeup". Pre-existing wakeup events in old sessions remain unchanged (no transcript version bump). (#376)
- `getMcpConfigService` extracted into `mcpConfigServiceRef` so non-entry main-process modules stop back-importing `main/index.ts` just to read the running MCP config service. Evaluating the entry point at module load drags in `autoUpdater`, `WorkspaceWatcher`'s `app.on('before-quit')`, `TerminalSessionManager`'s `app.isPackaged` reads, and other singletons that crash in vitest's node environment. `index.ts` keeps its local `mcpConfigService` variable and all 24 internal read sites untouched; external readers go through the ref module.
- `MessageStreamingHandler`-style listener tracking applied to internal vitest stability paths so `ElectronDocumentService` metadata tests no longer pull the full main-process graph through `TrackerSyncManager` -> database initializer -> `SyncManager` -> `WorkspaceWatcher` -> `WindowManager` -> `main/index`. The mocks for `PGLiteDatabaseWorker` and `TrackerSyncManager` match the pattern already used by the sibling `inlineIdentity` and `trackerSync` tests in the same directory.
...
- Tracker (kanban) tools now work from the agent. All 11 `tracker_*` tools (`tracker_list`, `tracker_get`, `tracker_list_types`, `tracker_create`, `tracker_update`, `tracker_link_session`, `tracker_unlink_session`, `tracker_link_file`, `tracker_add_comment`, `tracker_define_type`, `tracker_delete_type`) were missing from `INTERNAL_MCP_TOOLS` in `packages/runtime/src/ai/server/providers/claudeCode/toolPolicy.ts`. Without an entry, the SDK's `canUseTool` callback fell through `resolveImmediateToolDecision`'s auto-allow gate, the permission system had no UI wired for nimbalyst-owned MCP tools so the Promise never resolved, and the SDK surfaced `user cancelled MCP tool call` for every kanban interaction. Adding all 11 entries reuses the existing in-process auto-allow path that already covered `display_to_user`, `capture_editor_screenshot`, the developer git tools, and session-context tools. 11 regression tests in `immediateToolDecision.test.ts` exercise the real `INTERNAL_MCP_TOOLS` list against each tracker tool plus a third-party-fall-through guard. (#236)
...
- Claude usage indicator no longer hidden by a deprecated `navigator.platform` check. `navigator.platform` returns `""` in newer Chromium/Electron, so the `isMacOS` guard always failed and the indicator never rendered. Replaced with `process.platform` and dropped the macOS-only restriction since the service already supports Windows/Linux via `~/.claude/.credentials.json`. (#362)
- Marketplace clone-source path no longer bricks an existing GitHub install when the new clone is missing `dist/`. The clone-source path now validates the manifest and required `dist/` against the cloned source dir BEFORE removing the existing installation. Previously, a missing `dist/` on the new clone would delete the working install and then bail out, leaving the marketplace record pointing at a directory that no longer exists; a "reinstall" could brick a working extension. This path is hit more often now that release lookups can miss or 404 and fall through here. Additionally, `selectReleaseAsset` now matches release asset names against the LAST path segment of the subdir, not the full subdir string. GitHub release asset filenames are flat (no slashes), so URLs like `/tree/main/packages/extensions/mockuplm` previously always failed to match any asset and silently fell back to clone-source. Now they match an asset like `mockuplm-0.4.0.nimext`. 3 unit tests cover nested-path match, trailing slash, and no-match-returns-null for the subdir path.
- Manifest-only extensions (theme-only, `claudePlugin`-only) now install from a GitHub URL. The "Install from GitHub" flow in `ExtensionMarketplaceHandlers` unconditionally required a `dist/` directory, rejecting manifests that the runtime loader explicitly accepts without a main entry point. The `onlyThemes` / `onlyClaudePlugin` logic from `ExtensionLoader.ts` is now mirrored on the install side; the `dist/` check is gated on whether the manifest needs runtime JS. Behavior for JS-bearing extensions is unchanged. (#355, #356)
- Queued prompts no longer re-send on restart-after-AskUserQuestion, on user-initiated cancel mid-turn, and on mobile-sync rollback. The `queued_prompts` boot sweep and cancel/interrupt paths were blindly rolling `executing` rows back to `pending` even when the user message had already landed in `ai_agent_messages`; the follow-up queue trigger then re-claimed the row and re-sent the same prompt. New delivery-aware sweeps (`sweepExecutingOnBoot` for the boot path, `sweepExecutingForSession` for cancel/interrupt/mobile-sync) check `ai_agent_messages` for an input row dated at or after `claimed_at` and mark delivered rows `completed` instead of `pending`. The boot sweep also catches leftover corruption from older builds: `pending` rows whose prompt text appears in a later input, plus any `pending` row older than 24h (covers JSON-escape-miss false negatives and abandoned prompts). `cancelRequest`, `interruptCurrentTurn`, and the mobile-sync rollback helper now route to the new per-session sweep. (NIM-615)
- Lightning button reliably interrupts and drains the queue. Default `interruptCurrentTurn` on `BaseAIProvider` so every provider has a sensible behaviour (Claude Code keeps its true mid-stream interrupt; others abort) and the handler returns `{ method }` so callers can distinguish. `handleSendNowQueuedPrompt` now explicitly triggers queue processing after interrupting instead of depending on the `isLoading` auto-effect. Added `rollbackExecuting` / `rollbackAllExecuting` to the queued-prompts store so stuck rows reset on interrupt, cancel, mobile cancel, and at app startup — preventing a hang or crash mid-execute from permanently wedging the queue. Mobile prompt control message wired to `triggerQueuedPromptProcessing` so iOS prompts to an idle desktop actually fire. Edit/pencil appends to the draft input instead of clobbering it when multiple queued items are edited. (#337)
- Unsaved tab edits no longer get dropped when the underlying file is renamed. Document models now stay attached when tabs re-render or remount after a rename; rename collisions that would orphan an already-open destination file are blocked. (#367)
- CSV and datamodel shared documents survive close-and-reopen. Fixed blank CSV grid on reopen by stashing `pendingDataRef` in the collab `applyCsvContent` path so a fast WebSocket sync before grid mount is not clobbered by the lifecycle's empty `applyContent`. Added real-time collaboration to the DatamodelLM extension via a `Y.Doc`-backed binding with content-derived stable IDs. Added `flushLocalState` to `DocumentSync` + `CollaborationContext` so the SDK hook can push a freshly seeded `Y.Doc` upstream without waiting on observer/replay timing. Exported `createProxiedWebSocket` and added `__openCollabDocTest` dev helper so E2E tests can drive collab opens through wrangler with `test_user_id`/`test_org_id` auth bypass. E2E regression added: CSV collab doc content survives close+reopen (gated on `RUN_COLLAB_TESTS=1`).
- In-app issue flow now sets the correct GitHub type, labels, and template. The URL builder pointed at `bug_report.md` / `feature_request.md` (templates are `.yml`), used non-existent `bug` / `enhancement` labels, and passed the body via `?body=` which `.yml` issue forms ignore. The flow now routes to the `.yml` templates, drops the bogus labels query, and prefills the body into the template's primary textarea field (`problem` for bug, `request` for feature). GitHub issue type and `status:needs-triage` label now apply from the template frontmatter as intended. `version` and `os` fields are optional in `bug_report.yml` so the prefilled form is submittable without forcing the user to retype environment info that the agent already includes inline. Also adds an "Enable Usage Indicator" toggle to the Claude Agent settings panel mirroring the existing Codex toggle.
- Codex interactive prompt correlation stabilized. Centralized prompt id recovery and alias handling for Codex; routed `RequestUserInput` responses through shared fallback logic; documented the durable prompt identity model and regressions.
- Codex thread IDs preserved across blocked turns. New Codex thread IDs are now persisted before interactive prompts can block a turn; regression coverage added for restart-safe resume after blocked input.
- B2B auth callback now fails fast when Stytch returns no session instead of building a broken `nimbalyst://auth/callback?session_token=&session_jwt=...` deep link. Replaced the `|| ''` fallbacks in `authenticateB2BToken` with a `requireSession` helper that errors out when `session_token` or `session_jwt` is missing; MFA-required, password-required, and "member not authenticated" responses are distinguished so the rendered error page tells the user what actually went wrong. (#351, NIM-600)
- Alpha updates no longer 404 because of stray git tags landing at the top of `nimbalyst/nimbalyst`'s `releases.atom` feed. The `extension-sdk-v0.2.0` tag was picked up by `electron-updater`'s `GitHubProvider`, which asked for `latest-mac.yml` under a tag with no release assets. The `tags: extension-sdk-v*` push trigger is removed from the SDK publish workflow so SDK releases stop dirtying the app's tag namespace (`workflow_dispatch` only with an optional `version` input that must match `package.json`). Defense-in-depth: a `GitHubProvider` `httpRequest` shim now strips atom entries whose tag doesn't match `^v\d` before `electron-updater` parses the feed.
- SDK publish workflow coalesces duplicate runs for the same tag to prevent SDK publish failures from racing or duplicated workflow_dispatch invocations.
- Mac local builds unblocked and renderer / extension build-time warning noise reduced.
- Audit-file package count and licenses updated.
- Initial wave of follow-up CI fixes that landed alongside this release: math / SDK / collab extension package-lock updates and a broken-CI patch on top of the math editor rollout.

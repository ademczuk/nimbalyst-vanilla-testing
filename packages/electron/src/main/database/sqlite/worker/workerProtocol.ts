...
import type { MigrationRefusal } from '../migrationOutcome';
...
export interface MigrationPreflightResult {
  ok: boolean;
  reason?: string;
  /** Typed verdict when the failure is about the install, not the environment. */
...
/**
 * Identity and provenance of a migration/adoption run, generated by the main
 * caller so the terminal domain result can be deduped and attributed. See
 * `migrationOutcome.ts`.
 */
...
export interface StartMigrationPayload extends MigrationOperationPayload {
  userDataPath: string;
  schemaDir: string;
}

export interface StartDryRunPayload extends MigrationOperationPayload {
  userDataPath: string;
  schemaDir: string;
}

export interface AdoptDryRunPayload extends MigrationOperationPayload {
  userDataPath: string;
  schemaDir: string;
}

...
const MATERIALIZED_PATH = path.resolve(
  "/tmp/Nimbalyst Tutorial Materialized"
);
const SESSION_IDS = [
...
describe("seedTutorialSessions", () => {
  it("regenerates ids, rewrites materialized paths, and writes both tracker-link sides", async () => {
    ...
    const first = await seedTutorialSessions(MATERIALIZED_PATH, {
    ...
    const second = await seedTutorialSessions(MATERIALIZED_PATH, {
      getSessionStore: () => stores.sessionStore,
      getMessagesStore: () => stores.messagesStore,
      getFixtureDirectory: () => FIXTURE_DIRECTORY,
      generateSessionId: () => SESSION_IDS[idIndex++],
    ...
    expect(second.map((session) => session.id)).toEqual(SESSION_IDS.slice(2, 4));
    expect(new Set([...first, ...second].map((session) => session.id))).toHaveLength(4);
    ...
    const serializedMessages = stores.messages.map((message) => message.content);
    expect(serializedMessages.some((content) => content.includes("{{TUTORIAL_ROOT}}"))).toBe(false);
    expect(serializedMessages.some((content) => content.includes("{{SESSION_ID:"))).toBe(false);
    expect(
      serializedMessages.some((content) =>
        content.includes(path.join(MATERIALIZED_PATH, "documents/product-brief.md"))
      )
    ).toBe(true);
    expect(
    ...
    expect(linkTrackerSessions).toHaveBeenCalledWith(
      MATERIALIZED_PATH,
      "fm:bug:planning/bug-example.md",
      [SESSION_IDS[1]]
    );
    expect(first[1].metadata.linkedTrackerItemIds).toEqual([
      "fm:bug:planning/bug-example.md",
    ]);
  });
  ...
  it("loads a seeded session only for its real materialized workspace path", async () => {
    const stores = createStores();
    let idIndex = 0;
    const seeded = await seedTutorialSessions(MATERIALIZED_PATH, {
      getSessionStore: () => stores.sessionStore,
      getMessagesStore: () => stores.messagesStore,
      getFixtureDirectory: () => FIXTURE_DIRECTORY,
      generateSessionId: () => SESSION_IDS[idIndex++],

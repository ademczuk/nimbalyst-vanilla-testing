 ...
 *
 * The board's column derivation and drop writes.
 *
 * Two things here are invisible on screen and expensive to get wrong: that the
 * default (status) board still produces the exact schema-ordered column set it
 * always has, and that a milestone drag rewrites the `in-collection`
 * relationship instead of clobbering unrelated memberships.
 */

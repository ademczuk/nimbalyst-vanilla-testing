 ...
 * evidence through instead of just asserting the plan shipped.
 */
...
describe('plan status drift presentation', () => {
  ...
  it('hands over the commit and session evidence, and asks rather than asserts', () => {
    ...
    // The wording must not claim the plan shipped -- two of the four plans the
    // original audit called shipped did not survive reading the commits.
    expect(drift?.chipLabel).toMatch(/\?$/);
    expect(`${drift?.chipLabel} ${drift?.headline}`.toLowerCase()).not.toContain('shipped');
  });
});

...
import { resolveDisplayIssueKey, type IssueKeyStatus } from './localIssueKey';
...
function prepareGraph<T>(
  ...
  for (const node of nodesById.values()) {
    const fields = globalRegistry.get(node.type)?.fields ?? [];
...
export function trackerRecordReadinessAccessors(
  ...
  return {
    ...
    getStatus,
    getTitle: (record) => getRecordTitle(record) || undefined,
...
export function computeReadiness(
  ...
  return computeReadinessForItems(allItems, trackerRecordReadinessAccessors(getStatus));
}

...
export const SharedDocsListView: React.FC<SharedDocsListViewProps> = ({ folderId }) => {
  ...
  const allDocs = useAtomValue(session.atoms.sharedDocuments);
  const folders = useAtomValue(session.atoms.sharedFolders);
  const favorites = useAtomValue(session.atoms.favorites);
  const unreadIds = useAtomValue(session.atoms.changedDocumentIds);
  const openedAt = useAtomValue(session.atoms.openedAt);
  const receipts = useAtomValue(session.atoms.receipts);
  const currentUserId = useAtomValue(session.atoms.activeTeamUserId);
  const orgId = scope.orgId;
  const { personalState: personalStateAvailable, readReceipts: readReceiptsAvailable } =
    session.uiCapabilities;
  const visibleSegments = useMemo(() => SEGMENTS.filter((candidate) => {
    if (candidate.id === 'favorites' || candidate.id === 'recent') return personalStateAvailable;
    if (candidate.id === 'review') return readReceiptsAvailable;
    return true;
  ...
  const [sortColumn, setSortColumn] = useState<SortColumn>(
    readReceiptsAvailable ? 'needsReview' : 'lastEdited',
  );
  ...
  useEffect(() => {
    ...
    host.trackEvent?.('collab_home_opened', {
  ...
  const currentEmail = scope.indexConfig.userEmail ?? null;
  ...
  const typePresentation = useCallback(
    (doc: SharedDocument) => resolveSharedDocumentTypePresentation(doc, descriptors),
    [descriptors, documentTypesRevision],
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      host.trackEvent?.('collab_home_searched', {
        surface: host.surface ?? 'desktop',
  ...
  }, [filteredDocs.length, host, trimmedQuery]);
  ...
  const openDoc = useCallback((doc: SharedDocument) => {
    host.openArtifact({
      ...
      teamProjectId: doc.teamProjectId,
    }, 'home');
  }, [host, scope]);
  ...
  const copyLink = useCallback((doc: SharedDocument) => {
    ...
    host.trackEvent?.('collab_document_action', {
      ...
      action: 'link_copied',
      actorType: 'user',
      documentType: stableCategory(doc.documentType),
      entryPoint: 'home',
    });
  }, [host, orgId, scope]);
  ...
  /**
   * "No shared documents here yet." used to cover three unrelated situations —
   * an org with nothing in it, a search that missed, and a segment or facet
   * that filtered everything out. They need different things from the reader
   * (create something / retype / widen), so they say different things.
   */
  const emptyState = useMemo((): { icon: string; title: string; detail: string } => {
    if (trimmedQuery) {
      return {
        icon: 'search_off',
        title: `No matches for “${trimmedQuery}”`,
        detail: 'Search covers document names. Clear the search to see everything shared with you.',
      };
    }
    if (totalOpenable === 0) {
      return {
        ...
        detail: 'Documents your team shares appear here, and stay live for everyone who has them open.',
      };
    }
    const segmentLabel = visibleSegments.find((candidate) => candidate.id === segment)?.label;
    if (segment !== 'all' && segmentLabel) {
      return {
        icon: 'filter_list_off',
        title: `Nothing in ${segmentLabel}`,
  ...
  return (
    <div className="shared-docs-list-view h-full flex flex-col min-h-0 bg-nim select-text">
          ...
          {visibleSegments.map((seg) => {
            ...
            return (
              ...
              >
                ...
                {/* Wrapped so a reader of the DOM can get the label without the
                    icon's ligature text, which textContent otherwise prepends. */}
                <span className="shared-docs-segment-label">{seg.label}</span>
      ...
      <div className="shared-docs-table flex-1 overflow-y-auto min-h-0">
        {sortedDocs.length === 0 ? (
          <div className="shared-docs-empty flex flex-col items-center justify-center h-full text-center px-6 py-16">
            ...
            <p className="mt-1 mb-0 max-w-[26rem] text-[13px] leading-relaxed text-[var(--nim-text-muted)]">
              {emptyState.detail}
            </p>
          </div>
        ) : (
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr className="shared-docs-thead sticky top-0 z-10 bg-nim border-b border-nim text-[11px] uppercase tracking-wide text-[var(--nim-text-muted)]">
                ...
                {personalStateAvailable && <th className="w-8 px-2 py-2" />}
                    ...
                    {personalStateAvailable && (
                      <td className="px-2 py-2 align-middle">
                        <button
                          ...
                          onClick={(e) => { e.stopPropagation(); session.toggleFavorite(doc.documentId); }}
                        >
      ...
      <div className="shared-docs-list-footer flex items-center gap-2 px-4 py-1.5 border-t border-nim shrink-0 text-[11.5px] text-[var(--nim-text-muted)]">
        ...
        {readReceiptsAvailable && reviewCount > 0 && (
          <>
            <span className="text-[var(--nim-text-faint)]">·</span>

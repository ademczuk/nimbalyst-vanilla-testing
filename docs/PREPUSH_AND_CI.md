...
Only the final Vitest step can be reused. All build, typecheck, static, and Sandbox checks still execute locally. Reuse requires the exact full-suite argv, a complete PASS, a current Git/content fingerprint with matching Node major and platform/architecture, and a clean working tree. Every pushed ref must peel to the checked-out HEAD. Missing state, dirty files, changed inputs, unresolved/non-HEAD refs, and CI all run the suite. The existing no-new-commits shortcut is separate and unchanged.
...
Superseded pull-request runs are cancelled. Main runs are retained for release consumers; release and deployment workflows are unaffected. Fork PRs need only the read-only repository token. Exact caches include manifests, lockfiles, patches, platform/architecture, and build inputs; there are no partial restore keys. Vitest's file/test counts, elapsed time, and phase summary are saved in the job summary. Phase totals are accumulated across workers, not CPU time.
